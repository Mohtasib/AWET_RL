...
def Trainer(params):
    ...
    tensorboard_log_dir = f"experiments/{params['general_params']['env_name']}/{params['general_params']['exp_name']}/tensorboard_logs/"
    print('=========== To start TensorBoard:')
    ...
    for i in range(params['general_params']['num_runs']):
        ...
        if params['general_params']['agent'].startswith('DDPG'):
            ...
            model = DDPG(  'MlpPolicy', env, action_noise=action_noise, verbose=0, seed=s, 
        ...
        elif params['general_params']['agent'].startswith('TD3'):
            from stable_baselines3.common.noise import NormalActionNoise
            ...
            model = TD3(    'MlpPolicy', env, action_noise=action_noise, verbose=0, seed=s, 
                            gamma=params['algo_params']['gamma'], 
                            buffer_size=params['algo_params']['buffer_size'], 
                            learning_starts=params['algo_params']['learning_starts'], 
                            gradient_steps=params['algo_params']['gradient_steps'], 
        ...
        elif params['general_params']['agent'].startswith('SAC'):
            from stable_baselines3 import SAC

            model = SAC(    'MlpPolicy', env, verbose=0, seed=s, 
        ...
        elif params['general_params']['agent'].startswith('AWET_DDPG'):
            from stable_baselines3.common.noise import NormalActionNoise
            ...
            raise ValueError(f"The agent name must starts with 'DDPG', 'TD3', 'SAC', 'AWET_DDPG', 'AWET_TD3', or 'AWET_SAC' and not {params['general_params']['agent']}")

        # Create the callback: check every 1000 steps
        callback = SaveOnBestTrainingRewardCallback(check_freq=1000, log_dir=log_dir)

        # Train the agent
        if params['general_params']['agent'].startswith('DDPG') or params['general_params']['agent'].startswith('TD3') or params['general_params']['agent'].startswith('SAC'):
            model.learn(total_timesteps=int(timesteps), 
                        tb_log_name=f"{params['general_params']['agent']}_Seed_{s}", 
                        callback=callback)
        else:
            model.learn(total_timesteps=int(timesteps), 
